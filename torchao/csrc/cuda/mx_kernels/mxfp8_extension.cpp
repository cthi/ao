...
mxfp8_quantize(const Tensor& input, bool rowwise, bool colwise,
  ...
  if (rowwise) {
    ...
    output_rowwise = torch::stable::empty(
        ...
        input.device(),
        std::nullopt);
    scales_rowwise = torch::stable::empty(
        {rows, num_col_blocks},
  ...
  if (colwise) {
    ...
    // Need column-major layout for output_colwise
    // Using new_empty and reshape/transpose to achieve strided layout
    output_colwise = torch::stable::empty(
        {cols, rows},
        torch::headeronly::ScalarType::Float8_e4m3fn,
        torch::headeronly::Layout::Strided,
        input.device(),
        std::nullopt);
    output_colwise = torch::stable::transpose(output_colwise, 0, 1);
    ...
    scales_colwise = torch::stable::empty(
        {num_row_blocks, cols},
        torch::headeronly::ScalarType::Float8_e8m0fnu,
        torch::headeronly::Layout::Strided,
        input.device(),
        std::nullopt);
    scales_colwise = torch::stable::transpose(scales_colwise, 0, 1);
  } else {
    output_colwise = torch::stable::empty(
        {0},
        torch::headeronly::ScalarType::Float8_e4m3fn,
...
mxfp8_quantize_3d(const Tensor& input, int64_t scale_dim_n,
  ...
  // Allocate as (E, K, N) and transpose to get (E, N, K) with strides {N*K, 1, N}
  Tensor output_colwise = torch::stable::empty(
      {E, K, N},
      torch::headeronly::ScalarType::Float8_e4m3fn,
      torch::headeronly::Layout::Strided,
      input.device(),
      std::nullopt);
  output_colwise = torch::stable::transpose(output_colwise, 1, 2);

  // Create scales tensor with shape (E, num_n_blocks, K)
  const int64_t num_n_blocks = (N + scale_dim_n - 1) / scale_dim_n;
  Tensor scales_colwise = torch::stable::empty(
      {E, num_n_blocks, K},
      torch::headeronly::ScalarType::Float8_e8m0fnu,
      torch::headeronly::Layout::Strided,
      input.device(),
...
Tensor mx_block_rearrange_2d_M_groups(
  ...
  STD_TORCH_CHECK(scales_tensor.scalar_type() == torch::headeronly::ScalarType::Byte ||
  ...
  // Create output tensor - use new_zeros for zero-initialization
  auto output = torch::stable::new_zeros(
      scales_tensor,
  ...
  const int32_t* offsets_ptr = reinterpret_cast<const int32_t*>(input_group_end_offsets.data_ptr());
